...
void SD_SPI_Init(void)
{
  //设置硬件上与SD卡相关联的控制引脚输出
	...
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_4 | GPIO_Pin_8;//PA2.3.4 推挽 	n_3|GPIO_Pin_4;
...
//配置使能gd5801与TF卡的片选, 3257。 并将tf卡的片选设置为悬空
...
{
    //切换为GD5801与TF卡连接的流程            
    mcu_gd_set_gd();        //设置片选:1、主要将TF卡的片选设置为输入悬空；2、

    reset_pwr_gd_tf();      //将GD与TF的电源拉低，
    usart2_reset_tx_rx();   //设置MCU与GD的串口IO为推挽输出并拉低
    delay_ms(200);
          
	usart2_init();	//将MCU与GD的串口进行初始化（串口通信功能）
	
    set_pwr_gd_tf();        //将GD与TF的电源置高    
    delay_ms(100);
...
//		pd_log_buf(LOG_DEBUG, buf, 512);
	}
    else
    {
        printf("read Sector 0 err..\n");
    }
	myfree(buf);//释放内存	
}
//#endif
